...
class SearchEngine:
	...
	def evaluateDataset(self):
		...
		print(np.mean(nDCGs), np.mean(MAPs)) 
		...
		plt.xlabel("k")
		plt.savefig(args.out_folder + "Evaluation_metrics.png")
	...
	def gridsearch(self, plot_search = False):
		...
		min_k = 100
		step = 100
		ws = [0.5,0.7,0.9,1.0,1.1, 1.3, 1.5]
		count = 0

		for j in ws:
			for i,k in enumerate(np.arange(min_k,max_k,step)):
				count += 1
				print(f"Evaluating {count} of {((max_k-min_k)//step)*7}", end="\r")

				nDCG, fscore = self.just_evaluate(k, j)
				ks.append(k)
				nDCGs.append(nDCG)
				fscores.append(fscore)
				print('(',min_k+ step*i,',',j,')',':', nDCG)
